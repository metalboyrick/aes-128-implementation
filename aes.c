...
// GALOIS MULTIPLY TABLES from: https://en.wikipedia.org/wiki/Rijndael_MixColumns
...
void mix_columns(uint8_t* block){
	uint8_t res_block[16];
	for(int i = 0 ; i < 16; i++){
		if(i < 4) 
			res_block[i] = MUL_2[block[i]] ^ MUL_3[block[i+4]] ^ block[i+8] ^ block[i+12];
		else if(i >= 4 && i < 8)
			res_block[i] = block[i-4] ^ MUL_2[block[i]] ^ MUL_3[block[i+4]] ^ block[i+8];
		else if(i >= 8 && i < 12)
			res_block[i] = block[i-8] ^ block[i-4] ^ MUL_2[block[i]] ^ MUL_3[block[i+4]];
		else
			res_block[i] = MUL_3[block[i-12]] ^ block[i-8] ^ block[i-4] ^ MUL_2[block[i]];
	}
...
void inv_mix_columns(uint8_t* block){
	...
	for(int i = 0 ; i < 16; i++){
		if(i < 4) 
			res_block[i] = MUL_14[block[i]] ^ MUL_11[block[i+4]] ^ MUL_13[block[i+8]] ^ MUL_9[block[i+12]];
		else if(i >= 4 && i < 8)
			res_block[i] = MUL_9[block[i-4]] ^ MUL_14[block[i]] ^ MUL_11[block[i+4]] ^ MUL_13[block[i+8]];
		else if(i >= 8 && i < 12)
			res_block[i] = MUL_13[block[i-8]] ^ MUL_9[block[i-4]] ^ MUL_14[block[i]] ^ MUL_11[block[i+4]];
		else
			res_block[i] = MUL_11[block[i-12]] ^ MUL_13[block[i-8]] ^ MUL_9[block[i-4]] ^ MUL_14[block[i]];
	}
...
int encrypt(char* ptext){
	...
	printf("Encryption speed: %lf Megabits / second\n", speed);
